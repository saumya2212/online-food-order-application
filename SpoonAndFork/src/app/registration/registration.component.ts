...
export class RegistrationComponent {
  ...
  addUser(){
    ...
    this.userService.registerUser(fData).subscribe({
      next:data=>{
        ...
        body = `${this.name?.value} Your accout is created Successfully`;
        ...
        console.log(data);
        this._sanckBar.open('Account created successfully...', '', {
          duration: 3000,
  ...
  canDeactivate() {
    if (!this.submitStatus)
        this.submitStatus = confirm("You have not submitted a request to this page. Any details entered will be lost. Are you sure you want to leave?");
