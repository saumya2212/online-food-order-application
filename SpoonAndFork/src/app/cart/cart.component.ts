...
ngOnInit(): void {
  ...
  this.foodie.getItems("incart",localStorage.getItem('email')).subscribe((response:any)=>{
   
    this.restaurant.getAllRestaurant().subscribe((responseRes: any) => {
      let ress: [] = responseRes
      // response.items =[]
      let resId;
      
      const filteredRestaurants = ress.find((restaurant:any) =>
      response.every((item:any) =>
        restaurant.items.some(
          (restaurantItem:any) =>
            restaurantItem.itemId === item.itemId &&
            restaurantItem.itemName === item.itemName &&
            restaurantItem.itemPrice === item.itemPrice
        )
      )
    );
  
    const res:any=filteredRestaurants
    if(res){
     console.log(res,"restro");
     console.log(response,"item")
      // const ite: any[] =  res
      // const inv = ite.findIndex((item) => {
      //  return response.itemId === item.itemId && response.itemName === item.itemName
      // })
      // response[inv].restName = res?.restaurant
      const items: any[] = response
          const  resaurant = res
          const index = items.findIndex((item) =>
          resaurant.items.some(
            (restaurantItem: any) =>
              restaurantItem.itemId === item.itemId &&
              restaurantItem.itemName === item.itemName
          )
        );
        response[index].restName = res.restaurantName
        console.log(index,response,res.restaurantName,"responseee")
    }
    
     })
    console.log(this.restaurant.id,"restauurantId");
    this.items=response; 
    this.totalPrice();   
  },error=>{
    this.items=[];
...
//   }
  ...
  totalPrice() {
    ...
    // **Shipping charge removal:**
    this.subtotal = this.total; // Set subtotal directly to total
  }
